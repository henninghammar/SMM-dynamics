...
for k=1:2
    for m=1:2
        ...
        elseif tau < t1
                A(m,:) = -(1./2).*(exp(-1i.*(eps(m)-1i.*g(m)).*(tau-t0)-1i.*w.*t0).*(1./(w-eps(m)+1i.*g(m))-1./(w+eV(k)-eps(m)+1i*g(m)))...
                +exp(-1i.*eV(k).*(tau-t0)-1i.*w.*tau)./(w+eV(k)-eps(m)+1i*g(m)));
        else
                A(m,:) = -(1./2).*(exp(-1i.*(eps(m)-1i.*g(m)).*(tau)).*(1./(w-eps(m)+1i.*g(m)).*(...
                exp(1i.*(eps(m)-1i.*g(m)-w).*t0)+exp(1i.*(eps(m)-1i.*g(m)-w).*tau)-exp(1i.*(eps(m)-1i.*g(m)-w).*t1))...
        ...
        for l=1:2
            ...
            if tau < t0
                    C(m,l,:)=(1./4).*exp(-1i.*w.*tau)./((w-eps(m)+1i*g(m)).*(w-eps(l)+1i*g(l)));
            elseif tau < t1
                    C(m,l,:)=(1./4).*(exp(-1i.*w.*t0-1i.*(eps(m)-1i.*g(m)).*(tau-t0))./((w-eps(m)+1i*g(m)).*(w-eps(l)+1i*g(l)))...
                    +(exp(-1i.*w.*tau-1i.*eV(k).*(tau-t0))-exp(-1i.*w.*t0-1i.*(eps(m)-1i.*g(m)).*(tau-t0)))./((w+eV(k)-eps(m)+1i*g(m)).*(w+eV(k)-eps(l)+1i*g(l))));
            else
                    C(m,l,:) = (1./4).*(exp(-1i.*(eps(m)-1i.*g(m)).*(tau)).*(1./((w-eps(m)+1i*g(m)).*(w-eps(l)+1i*g(l))).*(...
                    exp(1i.*(eps(m)-1i.*g(m)-w).*t0)+exp(1i.*(eps(m)-1i.*g(m)-w).*tau)-exp(1i.*(eps(m)-1i.*g(m)-w).*t1))...
