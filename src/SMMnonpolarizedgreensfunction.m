function [G0less, G0great, G1xless, G1xgreat, G1yless, G1ygreat, G1zless, G1zgreat] = SMMnonpolarizedgreensfunction(t, tau, t0, t1, epsilon0, gamma0, gamma0lead, eV, w, fermi, S, J)
...
for k=1:2
    ...
    elseif tau < t1
            ...
            1./(w-epsilon0+1i.*gamma0/4)-1./(w+eV(k)-epsilon0+1i*gamma0/4))...
            +exp(-1i.*eV(k).*(tau-t0)-1i.*w.*tau)./(w+eV(k)-epsilon0+1i*gamma0/4);
    else
            A(k,:) = -(1./2).*(exp(-1i.*(epsilon0-1i.*gamma0/4).*tau).*(...
            ...
            +1./(w+eV(k)-epsilon0+1i*gamma0/4).*(...
            exp(1i.*(epsilon0-1i.*gamma0/4-w).*t1-1i.*eV(k).*(t1-t0))...
            -exp(1i.*(epsilon0-1i.*gamma0/4-w).*t0))));
    end
    ...
    elseif t < t1
            ...
            1./(w-epsilon0-1i.*gamma0/4)-1./(w+eV(k)-epsilon0-1i*gamma0/4))...
            +exp(1i.*eV(k).*(t-t0)+1i.*w.*t)./(w+eV(k)-epsilon0-1i*gamma0/4);
    else
    ...
    elseif tau < t1
            ...
            +(exp(-1i.*w.*tau-1i.*eV(k).*(tau-t0))...
            -exp(-1i.*w.*t0-1i.*(epsilon0-1i.*gamma0/4).*(tau-t0)))./((w+eV(k)-epsilon0-1i*gamma0/4).*(w+eV(k)-epsilon0-1i*gamma0/4));
    else
            ...
            +exp(1i.*(epsilon0-1i.*gamma0/4-w).*tau)...
            -exp(1i.*(epsilon0-1i.*gamma0/4-w).*t1))...
            +1./((w+eV(k)-epsilon0-1i*gamma0/4).*(w+eV(k)-epsilon0-1i*gamma0/4)).*(...
            exp(1i.*(epsilon0-1i.*gamma0/4-w).*t1-1i.*eV(k).*(t1-t0))...
            -exp(1i.*(epsilon0-1i.*gamma0/4-w).*t0)));
    end
    ...
    elseif t < t1
            ...
            +(exp(1i.*w.*t+1i.*eV(k).*(t-t0))...
            -exp(1i.*w.*t0+1i.*(epsilon0+1i.*gamma0/4).*(t-t0)))./((w+eV(k)-epsilon0-1i*gamma0/4).*(w+eV(k)-epsilon0-1i*gamma0/4));
    else
