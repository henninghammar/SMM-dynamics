...
%     G1zgreatint(j)=trapz(tau,G1zgreat);
    ...
    mx = 2*J*trapz(tau, G1xless);
    my = 2*J*trapz(tau, G1yless);
    mz = 2*J*trapz(tau, G1zless);
    ...
    dSx1=-(wL+mz-ejz).*Sy(j)+(my-ejy).*Sz(j)+trapz(tau,dSxbare);
    dSy1=-(mx-ejx).*Sz(j)+(wL+mz-ejz).*Sx(j)+trapz(tau,dSybare);
    dSz1=-(my-ejy).*Sx(j)+(mx-ejx).*Sy(j)+trapz(tau,dSzbare);
