...
J=0;%0.5*gamma; %Coupling strength
...
for i = 1:length(eVvector)
  ...
  [Icbias(i), Isxbias(i), Isybias(i), Iszbias(i), IeLbias(i), IqLbias(i), IeRbias(i), IqRbias(i), DOSbias(i,:), MDOSzbias(i,:)] = stationarycurrent(pL, pR, gamma, munew, eps, w, dw, J, S, wL, beta);
...
for i = 1:length(epsvector)
  ...
  [Icgate(i), Isxgate(i), Isygate(i), Iszgate(i), IeLgate(i), IqLgate(i), IeRgate(i), IqRgate(i), InLgate(i), DOSgate(i,:), MDOSzgate(i,:)] = stationarycurrent(pL, pR, gamma, mu, epsnew, w, dw, J, S, wL, beta);
  ...
  [T_up(i,:), T_down(i,:), GL(i), GR(i), GSL(i), GSR(i), ScL(i), ScR(i), SsL(i), SsR(i), kL(i), kR(i), ZTL(i), ZTR(i), ZTSL(i), ZTSR(i)] = thermoelectriccoefficients(pL, pR, gamma, mu, epsnew, w, dw, J, S, wL, beta, T);
end
